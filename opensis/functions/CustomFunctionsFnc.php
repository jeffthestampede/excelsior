...
{	
	...
	if(!$_REQUEST['delete_ok'] &&!$_REQUEST['delete_cancel'])
	{
		echo '<BR>';
		PopTable('header',$title);
		echo "<CENTER><h4>$question</h4><FORM name=prompt_form id=prompt_form action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST>$message<BR><BR><INPUT type=submit class=btn_medium value=OK onclick='formcheck_school_setup_copyschool();'>&nbsp;<INPUT type=button class=btn_medium name=delete_cancel value=Cancel onclick='load_link(\"Modules.php?modname=schoolsetup/Calendar.php\");'></FORM></CENTER>";
		PopTable('footer');
		return false;
	}
	else
...
{	
	...
	if(!$_REQUEST['delete_ok'] &&!$_REQUEST['delete_cancel'])
	{
		...
		echo '<table width="100%"><tr><td width="180px" align="right">Student Enrollment Date:</td><td>';

                echo DateInputAY('','roll_start_date','1');
                echo '<td></tr>';
                echo '<tr><td>';
                ...
                echo '<br>';
                echo '<table width="100%"><tr><td width="180px" align="right">School Begin Date :</td>';

                 echo '<td>'.DateInputAY('','roll_school_start_date','2').'</td>';

                 echo '<tr><td align="right">School End Date :</td><td>'.DateInputAY('','roll_school_end_date','3').'</td></tr></table>';
                echo '<hr>';
                ...
                echo '<input type=hidden id=prev_start_date value='.$prev_st_d[1]['END_DATE'].' >';
                    ...
                    {
                      $sem++;
                       echo '<table width="100%"><tr><td width="180px" align="right">'.$ss_d['TITLE'].' Begin Date :</td>';    

                        echo '<td>'.DateInputAY('','sem_start_'.$sem,$counter1);
                       echo '<input type=hidden id=name_'.$j.' value="'.$ss_d['TITLE'].' Begin Date" ></td>';
                       $j++;

                       $counter1=$counter1+1;
                        echo '<tr><td align="right">'.$ss_d['TITLE'].' End Date :</td><td>'.DateInputAY('','sem_end_'.$sem,$counter1);  
                       ...
                       if(count($check_sq)>0)
                        {   
                           ...
                           $p_val='';
                            ...
                            { $qrtr++;
                            $counter1=$counter1+1;
                               echo '<table width="100%"><tr><td width="180px" align="right">'.$sq_d['TITLE'].' Begin Date :</td>';    

                                echo '<td>'.DateInputAY('','qrtr_start_'.$qrtr,$counter1);
                               ...
                               echo '<tr><td align="right">'.$sq_d['TITLE'].' End Date :</td><td>'.DateInputAY('','qrtr_end_'.$qrtr,$counter1);   
                            ...
                            if(count($check_sp)>0)
                            {
                                ...
                                {$prog++;
                                 $counter1=$counter1+1;
                                   echo '<table width="100%"><tr><td width="180px" align="right">'.$sp_d['TITLE'].' Begin Date :</td>';    

                                    echo '<td>'.DateInputAY('','prog_start_'.$prog,$counter1);
                                   ...
                                   echo '<tr><td align="right">'.$sp_d['TITLE'].' End Date :</td><td>'.DateInputAY('','prog_end_'.$prog,$counter1);   
