...
function DeletePrompt_sys_maintain($title,$action='delete')
{
	...
	if(!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel'])
	{
		...
		PopTable('header','Confirm'.(strpos($action,' ')===false?' '.ucwords($action):''));
                    ...
                    echo "<CENTER><h4>Are you sure you want to $action that $title?</h4><br><FORM action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST><INPUT type=submit class=btn_medium value=OK>&nbsp;<INPUT type=button class=btn_medium name=delete_cancel value=Cancel onclick='window.location=\"Modules.php?modname=schoolsetup/SystemPreference.php&page_display=MAINTENANCE\"'></FORM></CENTER>";
		PopTable('footer');
		return false;
	}
	else
  ...
  $PHP_tmp_SELF1 = PreparePHP_SELF1($tmp_REQUEST1);
   $PHP_tmp_SELF =$PHP_tmp_SELF .$PHP_tmp_SELF1;
	if(!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel'])
	{
		echo '<BR>';
		PopTable('header','Confirm '.(strpos($action,' ')===false? ucwords($action):$action));
			echo "<CENTER><h4>Duplicate file found. There is already a file with the same name. Do you want to proceed?</h4><br><FORM action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST><INPUT type=button class=btn_medium value=OK onclick='load_link(\"Modules.php?modname=students/Student.php&include=FilesInc&category_id=7&stu_id=$_REQUEST[student_id]\");'>&nbsp;<INPUT type=button class=btn_medium name=delete_cancel value=Cancel onclick='load_link(\"Modules.php?modname=students/Student.php&include=FilesInc&category_id=7&delete_msg=yes&target_path=$target_path&stu_id=$_REQUEST[student_id]\");'></FORM></CENTER>";
		PopTable('footer');
		return false;
	}
	else
