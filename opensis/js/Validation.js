...
//	
    ...
    if (school_id != '')
    {
        ...
        frmvalidator.addValidation("inputvalues[ADDRESS]", "req", "Please enter address");
        frmvalidator.addValidation("inputvalues[ADDRESS]", "maxlen=100", "Max length for address is 100 characters");

        frmvalidator.addValidation("inputvalues[CITY]", "req", "Please enter city name");
        frmvalidator.addValidation("inputvalues[CITY]", "maxlen=100", "Max length for city is 100 characters");

        frmvalidator.addValidation("inputvalues[STATE]", "req", "Please enter state name");
        ...
        frmvalidator.addValidation("inputvalues[PRINCIPAL]", "req", "Please enter principal name");
        frmvalidator.addValidation("values[PRINCIPAL]", "maxlen=100", "Max length for state is 100 characters");
...
function validate_course_period()
{
    ...
    if (hidden_cp_id != 'new')
    {
        ...
        if (document.getElementById("inputtables[course_periods][" + hidden_cp_id + "][TOTAL_SEATS]").value == '')
            frmvalidator.addValidation("tables[course_periods][" + hidden_cp_id + "][TOTAL_SEATS]", "req", "Total Seat cannot be blank or '0'.");
    } else
    {
    ...
    frmvalidator.addValidation("tables[course_periods][new][TOTAL_SEATS]", "req", "Please input total seats");
    frmvalidator.addValidation("tables[course_periods][new][TOTAL_SEATS]", "count_check", "Total seats cannot be 0");
    frmvalidator.addValidation("tables[course_periods][new][TOTAL_SEATS]", "maxlen=10", "Max length for seats is 10");
...
function formcheck_grade_comment()
{
    ...
    if (comment_ids != '')
    {
        ...
        for (i = 0; i <= ar.length - 1; i++)
        {
            ...
            frmvalidator.addValidation("values[" + ar[i] + "][SORT_ORDER]", "req", "Sort order cannot be blank");
            frmvalidator.addValidation("values[" + ar[i] + "][TITLE]", "maxlen=50", "Max length for Comment is 50");
        }
    }
    if (document.getElementById('values[new][TITLE]').value != '' || document.getElementById('values[new][SORT_ORDER]').value != '')
    {
        ...
        frmvalidator.addValidation("values[new][SORT_ORDER]", "req", "Sort order is required");
    }
    ...
    for (var i = 0; i <= x; i++)
    {
        if (document.getElementById("qq").elements[i])
        {
            ...
            if (type == "checkbox")
            {
                if (document.getElementById("qq").elements[i])
                {
                    if (document.getElementById("qq").elements[i].name && document.getElementById("qq").elements[i].name != '')
                    {
