...
$edit_per_prnt = DBGet(DBQuery('SELECT CAN_EDIT FROM profile_exceptions WHERE MODNAME=\'students/Student.php&category_id=5\' AND PROFILE_ID=4'));
	...
	{	
		...
		{
			$header .= '<TR>';
			$header .= "<TD>&nbsp;</TD><TD align=right><a href='Modules.php?modname=students/Student.php&include=GoalInc&category_id=5&action=delete&gid=".$row_gid[1]["GOAL_ID"]."&pid=".$_REQUEST[progress_id]."'>Delete This Progress</a></TD>"; // DateInput is copied from schoolsetup/MarkingPeriods.php line 295
			$header .= '</TR>';
		}
	}
