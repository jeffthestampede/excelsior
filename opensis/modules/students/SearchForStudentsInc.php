...
if($_REQUEST['search_modfunc']=='search_fnc' || !$_REQUEST['search_modfunc'])
{
	...
	switch(User('PROFILE'))
	{
		...
		unset($_SESSION['s']);
			...
			echo "<FORM name=search id=search action=ForExport.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]&search_modfunc=list&next_modname=$_REQUEST[next_modname]".$extra['action']." method=POST target=_blank>";
			echo '<TABLE border=0>';
			Search('general_info');
			if($extra['search'])
				echo $extra['search'];
			...
			echo '<table><tr><td width="120" align="right">Comments </td><td><input type=text name="mp_comment" size=30 class="cell_floating"></td></tr></table><br />';
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>Birthday</b></div>';
			echo '<table><tr><td width="120" align="right">From: </td><td>'.SearchDateInput('day_from_birthdate','month_from_birthdate','','Y','Y','').'</td></tr>';
			echo '<tr><td align="right">To: </td><td>'.SearchDateInput('day_to_birthdate','month_to_birthdate','','Y','Y','').'</td></tr></table><br />';
