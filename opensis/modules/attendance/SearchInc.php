...
{       unset($_SESSION['new_sql']);
	...
	switch(User('PROFILE'))
	{
		...
		unset($_SESSION['s']);
			...
			echo "<FORM name=search id=search action=ForExport.php?modname=".strip_tags(trim($_REQUEST[modname]))."&modfunc=".strip_tags(trim($_REQUEST[modfunc]))."&search_modfunc=list&next_modname=$_REQUEST[next_modname]".$extra['action']." method=POST target=_blank>";
			echo '<TABLE border=0>';
			...
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>GoalInc and Progress</b></div>';
