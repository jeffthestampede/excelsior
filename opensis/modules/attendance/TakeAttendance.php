...
if($_REQUEST['attendance'] && ($_POST['attendance'] || $_REQUEST['ajax']))
{
	...
	if($_REQUEST['table']=='0')
	{
		...
		if(!count($RET))
                                    ...
                                    DBQuery('DELETE FROM missing_attendance WHERE  SCHOOL_DATE=\''.$date.'\' AND COURSE_PERIOD_ID=\''. UserCoursePeriod().'\'  AND(TEACHER_ID=\''.User('STAFF_ID').'\' OR SECONDARY_TEACHER_ID=\''.  User('STAFF_ID').'\')  AND PERIOD_ID=\''.UserPeriod().'\'');
	}
...
}else
{
 if(count($course_RET)!=0)
	{ 
	    echo '<div class='.(count($categories_RET)>0?"block_bg_div":"block_bg_div_noborder").'>';
		...
		ListOutput($stu_RET,$LO_columns,$singular,$plural,array(),array(),$extra);
                echo '</br>';
                if(count($stu_RET)>0 && count($course_RET)>0)
                echo '<center>'.SubmitButton('Save','','class=btn_medium').'</center><br/>';
		echo '</div>';	
	}
	else
        {
