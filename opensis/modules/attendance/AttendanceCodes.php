  ...
  echo '<input type="hidden" name="h1" id="h1" value="'.$attandance_id.'">';
    ...
    if($_REQUEST['table']!=='new' && $_REQUEST['modfunc']!='edit') {
        if(count($attendance_codes_RET)==0) {
            ...
            echo "<div id='students' >"; echo PopTable('header',$tabs); 
           
            ListOutput($attendance_codes_RET,$columns,'','',$link,array(),array('download'=>false,'search'=>false));  echo "</div>";         
            if($_REQUEST['table']!=0)
            {
            echo '<BR><CENTER><table cellpadding="5"><tr><th>'.button('edit','Edit category title',"Modules.php?modname=$_REQUEST[modname]&modfunc=edit&table=$_REQUEST[table]").'</th>';
            echo '<th>'.button('remove','Delete this category',"Modules.php?modname=$_REQUEST[modname]&modfunc=remove&table=$_REQUEST[table]").'</th></tr></table></CENTER>';
            }
    ...
    elseif($_REQUEST['table']=='new' && $_REQUEST['modfunc']!='edit') {
        ...
        echo '<CENTER>New Category Title <INPUT type=text id=new_category_title name=new_category_title ></CENTER>';
