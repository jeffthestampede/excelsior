...
if($_REQUEST['course_modfunc']=='search')
{
	...
	echo '<TABLE><TR><TD><INPUT type=text class=cell_floating name=search_term value="'.strip_tags(trim($_REQUEST['search_term'])).'"></TD><TD><INPUT type=submit class=btn_medium value=Search onclick=\'formload_ajax("F1")\';></TD></TR></TABLE>';
	echo '</FORM>';
	PopTable('footer');
...
if((!$_REQUEST['modfunc'] || $_REQUEST['modfunc']=='choose_course') && !$_REQUEST['course_modfunc'])
{
	...
	if($_REQUEST['subject_id'] && $_REQUEST['subject_id']!='new')
	{
		...
		if($_REQUEST['course_id'] && $_REQUEST['course_id']!='new')
		{
                ...
                if(count($periods_RET))
                {
                        echo '<BR>'.SubmitButton('Done','done','class=btn_medium ').'&nbsp;&nbsp;'.SubmitButton('Clear&Exit','exit','class=btn_medium ');
                }
                echo '</TD>';
            echo '</FORM>';
		}
