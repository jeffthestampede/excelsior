...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHA) =='update')
{
    ...
    foreach($_REQUEST['values'] as $request_id=>$columns)
	{
		...
		DBQuery($sql);
         ...
         if($flg==1)
                {
                    echo "<font color='red'>"."Teacher Contradiction."."</font>";
                         unset($_REQUEST['modfunc']);
                    
                }
                if($flg==2)
                {
                    echo "<font color='red'>"."Period Contradiction."."</font>";
                         unset($_REQUEST['modfunc']);
                    
                }
                if($flg==3)
                {
                    echo "<font color='red'>"."Teacher & Period Contradiction"."</font>";
                         unset($_REQUEST['modfunc']);
...
if(!$_REQUEST['modfunc'] && UserStudentID())
{
    ...
    {
	echo '<br><br><FORM name=ad id=ad action=Modules.php?modname='.strip_tags(trim($_REQUEST['modname'])).'&modfunc=add method=POST>';
	...
	echo '<br><br><FORM name=up id=up action=Modules.php?modname='.strip_tags(trim($_REQUEST['modname'])).'&modfunc=update method=POST>';
