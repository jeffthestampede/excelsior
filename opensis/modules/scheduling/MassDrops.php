                ...
                if(count($_REQUEST['student'])>0)
                {
                    foreach($_REQUEST['student'] as $student_id=>$yes)
                    {
                        ...
                        if($inactive_schedule_found==1)
                            DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'.$inactive_schedule.' has later schedule date');
...
if(!$_REQUEST['modfunc'])
{
       if($_REQUEST['search_modfunc']=='list')
        ...
        $extra['search'] .= "<TR><TD align='left' width='160' valign='top'>Course Period</TD><TD valign='top'><DIV id=course_div></DIV> <A HREF=# onclick='window.open(\"ForWindow.php?modname=$_REQUEST[modname]&modfunc=choose_course\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>Choose </A></TD></TR>";
         ...
         if($_REQUEST['search_modfunc']=='search_fnc' || !$_REQUEST['search_modfunc'])
        {
            ...
            echo "<FORM name=search id=search action=Modules.php?modname=".strip_tags(trim($_REQUEST[modname]))."&modfunc=".strip_tags(trim($_REQUEST[modfunc]))."&search_modfunc=list&next_modname=$_REQUEST[next_modname]".$extra['action']." method=POST>";
