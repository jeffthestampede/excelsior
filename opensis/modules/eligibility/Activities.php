...
if($_REQUEST['values'] && ($_POST['values'] || $_REQUEST['ajax']))
{
		...
		{$cnt=0;
			...
			foreach($columns as $column=>$value)
			{
                              if($column=='TITLE')
                               ...
                               if($title=='')
                            {
                            $err='<font style="color:red"><b>Cannot add activity with blank title.</b></font>';
                            $cnt=1;
			...
			$sql = substr($sql,0,-1) . ' WHERE ID=\''.$id.'\'';   
                        ...
                        $check_rec=DBGet(DBQuery('SELECT COUNT(*) as REC_EX FROM eligibility_activities WHERE UPPER(TITLE)=\''.$title.'\' AND ID!=\''.$id.'\' AND SYEAR=\''.UserSyear().'\'  AND SCHOOL_ID=\''.UserSchool().'\''));
                            ...
                            if($s_date1=='' || $e_date1=='')
                            {
                                $err='<font style="color:red"><b>Start date or End date cannot be blank title.</b></font>';
                           $cnt=1;
                                }
                            if($s_date1>$e_date1)
                        {
                          $err='<font style="color:red"><b>End date must be greater than begin date.</b></font>'; 
                          $cnt=1;
                        }
                        if($cnt==0)
                        {
  ...
  $err='<font style="color:red"><b>Cannot add activity with same title.</b></font>';   
 }
                        }
			...
			if($go)
                        {
                            ...
                            {
                                $err='<font style="color:red"><b>Start date or End date cannot be blank.</b></font>';
                           $cnt=1;
                                }
...
{	global $THIS_RET;
	...
	return TextInput($value,'values['.$id.']['.$name.']','','class=cell_floating maxlength=20');
}
