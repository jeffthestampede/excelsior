	...
	{  
 ...
 else 
     {
      echo '<BR>';
		...
		echo "<CENTER><h4>Please select atleast one student</h4><br><FORM action=$PHP_tmp_SELF METHOD=POST><INPUT type=button class=btn_medium name=delete_cancel value=OK onclick='load_link(\"Modules.php?modname=".strip_tags(trim($_REQUEST[modname]))."\");'></FORM></CENTER>";
		PopTable('footer');
		return false;
     }
	}
	else
        {
             echo '<BR>';
		PopTable('header','Alert Message');
		echo "<CENTER><h4>Please select atleast one activity</h4><br><FORM action=$PHP_tmp_SELF METHOD=POST><INPUT type=button class=btn_medium name=delete_cancel value=OK onclick='load_link(\"Modules.php?modname=".strip_tags(trim($_REQUEST[modname]))."\");'></FORM></CENTER>";
		PopTable('footer');
		return false;
        }
}
...
{
	echo "<FORM name=addact id=addact action=Modules.php?modname=".strip_tags(trim($_REQUEST[modname]))."&modfunc=save METHOD=POST>";
