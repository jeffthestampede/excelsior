    ...
    if ($_POST['button'] == 'Save' && $conflict === true) {
        if ($_REQUEST['mode'] == 'add') {
            ...
            if ($chek_assoc[1]['REC_EX'] == 0) {
                ...
                $fields = 'COURSE_PERIOD_ID,COURSE_PERIOD_DATE,';
                $values = "'" . $_REQUEST['course_period_id'] . "','" . $_REQUEST['meet_date'] . "',";
                $go = false;
...
if (!$_REQUEST['modfunc'] && !$_REQUEST['course_modfunc'] && !$_REQUEST['action']) {
    ...
    if (clean_param($_REQUEST['course_period_id'], PARAM_ALPHANUM)) {
        ...
        if ($_REQUEST[course_period_id] == 'new')
            ...
            $cal_RET = $cal_RET[1];
            $header .= '<TD>' . SelectInput($cal_RET['CALENDAR_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][CALENDAR_ID]', 'Calendar', $options, 'N/A', ' id=calendar_id onchange=reset_schedule();', $div) . '</TD>';
        }
        ...
        $header .= '<TD>' . SelectInput($RET['GRADE_SCALE_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][GRADE_SCALE_ID]', 'Grading Scale', $options, 'Not Graded', 'onChange="toggle_course_weight(this,' . $cp_id_js . ');"', $div) . '</TD>';
        ...
        $header .= '<TD>' . CheckboxInput($RET['DOES_BREAKOFF'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][DOES_BREAKOFF]', 'Allow Teacher Gradescale', $checked, $new, '<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>', '<IMG SRC=assets/y.gif height=15 vspace=0 hspace=0 border=0>', $div, 'id="course_breakoff_id" ' . ($RET['GRADE_SCALE_ID'] == '' ? 'disabled="disabled"' : '')) . '</TD>';
        $header .= '</TR><TR>';
        $header .= '<TD>' . CheckboxInput($RET['COURSE_WEIGHT'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][COURSE_WEIGHT]', 'Course is Weighted', $checked, $new, '<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>', '<IMG SRC=assets/y.gif height=15 vspace=0 hspace=0 border=0>', $div, 'id="course_weight_id" ' . ($RET['GRADE_SCALE_ID'] == '' ? 'disabled="disabled"' : '')) . '</TD>';
        ...
        $header .= '<TD valign=top>' . CheckboxInput($RET['HALF_DAY'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][HALF_DAY]', 'Half Day', $checked, $new, '<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>', '<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>', $div,'disabled id="half_day"') . '</TD>';
        else
        $header .= '<TD valign=top>' . CheckboxInput($RET['HALF_DAY'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][HALF_DAY]', 'Half Day', $checked, $new, '<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>', '<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>', $div,'id="half_day"') . '</TD>';    
        }
        else
        $header .= '<TD valign=top>' . CheckboxInput($RET['HALF_DAY'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][HALF_DAY]', 'Half Day', $checked, $new, '<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>', '<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>', $div,'disabled id="half_day"') . '</TD>';    
        ...
        $header .= '<TD colspan=4 width="550px"><TABLE><TR><TD><input type=radio name=date_range value=dr id=custom onchange=mp_range_toggle(this); ' . ($RET['BEGIN_DATE'] ? ' checked' : '') . '></TD><TD><label for=custom id=select_range style="display:' . ($RET['BEGIN_DATE'] ? 'none' : 'block') . '">Custom Date Range</label><DIV id=date_range style=display:' . ($RET['BEGIN_DATE'] ? 'block' : 'none') . '><TABLE><TR><TD>' . DateInputAY($RET['BEGIN_DATE'], 'begin', 1,($_REQUEST['conflict']=='y'?true:false)) . '</TD><TD> &nbsp; &nbsp; &nbsp; </TD><TD>  ' . DateInputAY($RET['END_DATE'], 'end', 2,($_REQUEST['conflict']=='y'?true:false)) . '</TD></TR></TABLE> </DIV></TD></TR></TABLE></TD></TR></TABLE></TD></TR>';
        ...
        elseif ($RET['SCHEDULE_TYPE'] == 'VARIABLE') {
           ...
           if ($_REQUEST['course_period_id'] != 'new') {
            ...
            foreach ($caldays as $day => $short_day) {
                ...
                $header .= '<TD>' . CheckboxInput($value, 'tables[course_period_var][' . $_REQUEST['course_period_id'] . '][DAYS][' . $day . ']', ($day == 'U' ? 'U' : $day), $checked, false, '', '', false) . '</TD>';
            }
            $header .= '</TR></TABLE>';
            if ($not_pass != true)
        ...
        elseif ($RET['SCHEDULE_TYPE'] == 'BLOCKED') {
            ...
            for ($i = 1; $i <= $last; $i++) {
                ...
                $header .= "<TD  title='" . ProperDate($date) . "' width=100 class=" . ($periods[$cpblocked_RET[$date][1]['PERIOD_ID']] || $calendar_RET[$date1][1]['MINUTES'] == '999' ? 'calendar_active' : 'calendar_holiday') . " valign=top>
