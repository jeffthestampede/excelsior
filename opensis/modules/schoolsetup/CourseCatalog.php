...
else
{
    ...
    if($_REQUEST['print']!='list')
    {
        echo "<FORM name=search id=search action=ForExport.php?modname=$_REQUEST[modname]&modfunc=print&marking_period_id=".$_REQUEST['marking_period_id']."&_openSIS_PDF=true&report=true&print=list method=POST target=_blank>";
...
function CreateList($dli='', $pli='', $sli='', $cli='', $mp='', $mp_name='')
{
   ...
   if($_REQUEST['print']=='list')
    ...
    ListOutputFloat($ret,$columns,'Course','Courses','','',array('search'=>false,'count'=>false),false);
	echo '</div>';
    echo "<div style=\"page-break-before: always;\"></div>";
    ##########################################################################################################
}
...
function CreateExcel($dli='', $pli='', $sli='', $cli='', $mp='', $mp_name='')
{
    ...
    $column_names = array('SUBJECT'=>'Subject','COURSE'=>'Course','SHORT_NAME'=>'Class Name', 'PERIOD'=>'Period','TEACHER'=>'Teacher', 'LOCATION'=>'Location','DAYS'=>'Days','BEGIN_DATE'=>'Start Date','END_DATE'=>'End Date' );
