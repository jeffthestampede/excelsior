                    ...
                    if($id!='new')
                        ...
                        $asso_err=false;
                                ...
                                else 
                                {
                                        ...
                                        {
                                           $TITLE_COUNT =  DBGet(DBQuery('SELECT * FROM marking_periods WHERE upper(TITLE)=\''.strtoupper(str_replace("'","''",$value)).'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\'AND MARKING_PERIOD_ID NOT IN(\''.$_REQUEST['marking_period_id'].'\')'));
			...
			else
			{
				...
				foreach($columns as $column=>$value)
				{
					...
					if($value)
					{
                                         if($column=='TITLE' && $columns['TITLE']!='')
                                        {
                                            $TITLE_COUNT =  DBGet(DBQuery('SELECT * FROM marking_periods WHERE upper(TITLE)=\''.strtoupper(str_replace("'","''",$value)).'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\''));
                                            if(count($TITLE_COUNT)>0)
                                            {
                                                $err_msg='Title alreday exsits';
