                                ...
                                if(!(isset($columns['TITLE']) && trim($columns['TITLE'])==''))
                               {
                                    ...
                                    $flag=1;
                                     ...
                                     ##############end#####################################    
                                            ...
                                            {
                                               echo "<font color='red'><b> Unable to save data, because break of already exists.</b></font>";
                                               break ; 
                                            }
                                            else
                                            {
...
//                                                        
                                                ...
                                                if($go)
                                                {
                                                     ...
                                                     if($validate_title[1]['TITLE_EX']!=0 || $new_cat=='attendance')
                                                     {
                                                      ...
                                                      if($v_break>0)
                                                         {
                                                        echo "<font color='red'><b> Unable to save data, because break of already exists.</b></font>";break ; 
                                                         }
                                                     else
                                                     {
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='remove')
{
 if($_REQUEST['tab_id']!='new'){
	...
	if($_REQUEST['tab_id']!='new')
	{
		...
		echo "<CENTER><h4>Already associated with a course period</h4><br><FORM action=$PHP_tmp_SELF METHOD=POST><INPUT type=button class=btn_medium name=delete_cancel value=OK onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]\");'></FORM></CENTER>";
		PopTable('footer');
		return false;
                    }
