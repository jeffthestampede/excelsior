...
if(UserStudentID())
{
    ...
    if ($_REQUEST['modfunc']=='update' && !$_REQUEST['removemp']){
        ...
        foreach($_REQUEST['values'] as $id=>$columns)
        {
            ...
            if($f==1)
        {
            echo '<div id="divErr"><FONT style="color:red">Credit Earned can be only equal to Credit Attempted or equal to 0.</FONT></div>';
            break;
        }
        }
  ...
  $stu_gpa_cal=DBGet(DBQuery('SELECT COUNT(*) AS COUNT FROM student_gpa_calculated WHERE marking_period_id=\''.$stu_val['MARKING_PERIOD_ID'].'\' AND student_id=\''.$stu_val['STUDENT_ID'].'\''));
    ...
    if(!$_REQUEST['modfunc']){    
       ...
       if($mp_id)
        ...
        $mpselect = "<FORM action=Modules.php?modname=$_REQUEST[modname]&tab_id=".$_REQUEST['tab_id']." method=POST id=F2 name=F2>";
