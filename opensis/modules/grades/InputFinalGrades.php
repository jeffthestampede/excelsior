...
{
        echo ErrorMessage(array('You cannot enter letter grades as grade scale is not set for this course .'),'error');
        $not_graded=true;
                          ...
                          if(count($get_details1)!=count($default_details1))
                            {
                           	...
                           	DrawHeader('<FONT COLOR=red>Score Breakoff Points setup is incomplete,Please set Score Breakoff Points from configuration.</FONT>');
                            }
	...
	elseif($not_graded!=true)
		...
		if(substr($_REQUEST['mp'],0,1)!='E')
		{
                        ...
                        if($cp_mp=='SEM' && $cp_type!='custom' && $prev_mp)
			{
				...
				$gb_header .= " | <A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=comments&mp=$_REQUEST[mp]&prev_mp=$prev_mp[MARKING_PERIOD_ID]&use_percents=false&period=$_REQUEST[period]>Get $prev_mp[TITLE] Comments</A>";
			}
		}
                                    if($cp_type!='custom')
                                    $bar=' | ';
                                    ...
                                    if(substr($_REQUEST['mp'],0,1)=='E')
                                        $bar=' | ';
                                            $gb_header .= str_replace(' |  | ',' | ',$bar)."<A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=clearall&mp=$_REQUEST[mp]&use_percents=$_REQUEST[use_percents]&period=$_REQUEST[period]>Clear All</A>";
	}
