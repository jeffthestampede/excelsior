	...
	foreach($_REQUEST['tables'] as $id=>$columns)
	{
		...
		if($id!='new')
		{
			...
			foreach($columns as $column=>$value)
			{
                                ...
                                if(($column=='DUE_DATE'|| $columns=='ASSIGNED_DATE')&& $value!='' && $table=='gradebook_assignments')
                                {
                                   
                               
$grade_assign_qr=  DBGet(DBQuery("SELECT COUNT(STUDENT_ID) AS TOT FROM  gradebook_grades WHERE ASSIGNMENT_ID='$id'"));
...
$grade_assign_qr=  DBGet(DBQuery("SELECT COUNT(STUDENT_ID) AS TOT FROM  gradebook_grades WHERE ASSIGNMENT_ID='$id'"));
                        ...
                        {
                            $f=1;
                            ShowErrPhp('Cannot modify the points because grade has been assign to the student for this assignment.');
                        }
                         }
                ...
                if((isset($columns['DUE_DATE']) || isset($columns['ASSIGNED_DATE'])) && $err==false)
                {
                    ...
                    if(isset($columns['DUE_DATE']) && !isset($columns['ASSIGNED_DATE']))
                    {
                        ...
                        if($get_custom_date[1]['MARKING_PERIOD_ID']==NULL) 
                       {
                        ...
                        {
                            $msg='<Font color=red>Due date must be within the course periods start date and end date.</FONT>';
                            $go=false;
                            $_REQUEST['assignment_id'] = 'new'; 
                        }  
