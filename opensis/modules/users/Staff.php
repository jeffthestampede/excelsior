...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='update')
 ...
 $this_school_RET_mod = DBGet(DBQuery("SELECT s.*,l.* FROM staff s,login_authentication l  WHERE l.USER_ID=s.STAFF_ID AND l.PROFILE_ID NOT IN (3,4) AND s.STAFF_ID=".$_REQUEST[staff_id]));
   ...
   if($_FILES['file']['name'])
                                    {
     echo  $UserPicturesPath='assets/userphotos/';
...
if((UserStaffID() || $_REQUEST['staff_id']=='new')  && ((basename($_SERVER['PHP_SELF'])!='index.php') || !$_REQUEST['staff']['USERNAME']) && $_REQUEST['modfunc']!='delete' && $_SESSION['fn']!='user' && $_REQUEST['modfunc']!='remove')
 { 
       ...
       if($_REQUEST['staff_id']!='new')
	{
		...
		echo "<FORM name=staff id=staff action=Modules.php?modname=$_REQUEST[modname]&custom=staff&include=$_REQUEST[include]&category_id=$_REQUEST[category_id]&staff_id=".UserStaffID()."&modfunc=update method=POST enctype='multipart/form-data'>";
	}
       elseif(basename($_SERVER['PHP_SELF'])!='index.php')
		echo "<FORM name=staff id=staff action=Modules.php?modname=$_REQUEST[modname]&include=$_REQUEST[include]&category_id=$_REQUEST[category_id]&modfunc=update method=POST enctype='multipart/form-data'> ";
                else
