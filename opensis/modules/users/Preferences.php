...
if(!$_REQUEST['modfunc'])
{
	...
	if(clean_param($_REQUEST['tab'],PARAM_ALPHAMOD)=='student_listing')
	{
		...
		echo '<TR><TD colspan=2><INPUT type=checkbox onClick="toggle_div_visibility(\'show_other_options\',this,\'json_encoder\');" name=values[Preferences][SEARCH] value=Y'.((Preferences('SEARCH')=='Y')?' CHECKED':'').'>Display student search screen</TD></TR>';
		if(User('PROFILE')=='admin')
		{
                        echo '<TR><TD colspan=2><div id="show_other_options" '.((Preferences('SEARCH')=='Y')?'style="display:block"':'style="display:none"').'><TABLE>';
			echo '<TR><TD><INPUT type=checkbox id="family" name=values[Preferences][DEFAULT_FAMILIES] value=Y'.((Preferences('DEFAULT_FAMILIES')=='Y')?' CHECKED':'').'>Group by family by default</TD></TR>';
			echo '<TR><TD><INPUT type=checkbox id="all_school" name=values[Preferences][DEFAULT_ALL_SCHOOLS] value=Y'.((Preferences('DEFAULT_ALL_SCHOOLS')=='Y')?' CHECKED':'').'>Search all schools by default</TD></TR>';
